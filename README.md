...
> 数据型(C) 走 Analyst→Scout→Modeler→Solver→Critic→Writer；**优化型(B) 自动改走** Analyst→Modeler→Solver→Critic→Writer（无重型 Scout）。
...
├── cases/
│   ├── 2024_mcm_c_tennis/      # demo①：网球动量（数据型 / 体育）
│   ├── credit_default_fintech/ # demo②：信用卡违约预测（数据型 / 金融风控）
│   └── 2024_logistics_siting/  # demo③：应急服务站选址（优化型 / 运筹，证明换题型即用）
├── tools/check_frozen.py    # 数字可追溯校验（论文数字↔脚本输出，24/24）
├── .claude/agents/          # 7 角色的 Claude Code subagents 移植版（已端到端验证）
