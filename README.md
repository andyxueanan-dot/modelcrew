...
> 🧪 **端到端自跑实证**：`cases/demo_dispatch_simpson/`（调度策略评估，埋了辛普森混杂）是用升级后的 **8 角色子代理自己从头跑完**的——Router→Analyst→Scout→Modeler→Solver→Critic→Writer→Judge 全链路真跑：Solver 先跑 **PoC 冒烟闸**(PASS) 再三法求解；⭐**Critic 独立从原始 CSV 复算、抓出散文里 3 处旧数字并触发修正**（`CORRECTION_audit.md`）、并把"方向≠显著"措辞红线立好；国赛 `6_paper.tex` 用 **xelatex 真编译出 13 页 PDF**；Judge 给 **83/100**（预估国二偏上，非官方）。这是对全部升级最硬的"它自己跑、自己纠错"实战检验。
...
python tools/check_frozen.py        # 当前 33/33 一致
```
...
│   └── demo_dispatch_simpson/  # demo④：调度策略评估（数据型 / 辛普森混杂）——8 角色端到端自跑+自纠，国赛 .tex 真编译 13 页
├── tools/check_frozen.py    # 数字可追溯校验（论文数字↔脚本输出，33/33）
