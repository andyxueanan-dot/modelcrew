...
不做一次性分析，而是造一支 **7 角色、可复用的数学建模 AI 团队**：
...
它是**建模副驾(copilot)/脚手架**，自动包办可复用的流程——审题、选方法、搭框架、查错、套论文格式；
**硬核的建模灵感仍由人(我)来把舵**。"通用"指流程通用，不是"全自动解出任何题"。

---

## 框架：7 角色 AI 团队（`agents/`，题目无关、可复用）
...
| 6 | **Writer 写作** | 按竞赛口味写报告，如实写局限 |

## 样例：`cases/2024_mcm_c_tennis/`

用网球动量题验证框架。选它因为 Q2"连胜是否随机"正中 **热手谬误**——Critic 的完美舞台。
Router 路由路线：Analyst → Scout(重) → Modeler(EWMA) → Solver → ⭐Critic → Writer。
...
qoder/
├── PLAN.md / README.md
├── agents/                      # ★ 可复用的 7 角色团队源稿（0_router.md … 6_writer.md）
├── skills/                      # 7 个 modelcrew-* 技能（Qoder SKILL.md 备份）
├── references/                  # 6 份借鉴资产（模型目录/反模式/评分/四层反馈/写作模板/相关工作）
├── cases/
