...
\subsection{Q3+ · 弱效应招式箱：把``未达显著''量化成决策价值}
三法 95\% 区间均跨零、属``方向稳健但未达显著''。本文不停在``做不出结论''，以三招把不确定性量化（绝不夸大为显著）：
\begin{center}
\begin{tabular}{p{3cm}p{6cm}p{4.5cm}}
\toprule
招 & 量 & 结果 \\
\midrule
功效/样本量反算 & 达 80\% 功效（$\alpha=0.05$ 双侧）检出当前约 4.5 pp 标准化率差所需样本量 & 每组 $N=1706$（合计 $3412$），现有 700；Cohen's $h=0.096$ \\
TOST 等效检验 & 实质等效边界 $\pm5$ pp / $\pm10$ pp（先验设定） & $\pm5$ pp：$p=0.45$ 不等效；$\pm10$ pp：$p=0.10$ 不等效 \\
E-value（因果稳健性） & 推翻 logistic OR(B/A)$=1.323$ 方向所需未观测混杂强度（结局常见，RR$\approx\sqrt{\text{OR}}$） & 点估计 E-value$=1.566$；区间含零 $\to$ E-value(CI)$=1.0$ \\
\bottomrule
\end{tabular}
\end{center}
三招一致表明``B 略优''是\textbf{方向稳健、强度证据不足}：既非``无效应''（招 1/2 显示是样本不足），方向又\textbf{对未观测混杂不稳健}（招 3：一个 RR$\approx$1.57 的未观测混杂即可解释掉它）。这把``未达显著''转译为\textbf{做出结论所需的样本量与结论的脆弱程度}，决策收口见第 7 节。
...
\section{模型的评价}
\subsection{优点}
(1) 叙事链完整且可证伪：Q1（陷阱）$\to$ Q2（反转判定准则，0/1 可判定）$\to$ Q3（三法校正），每步配``为什么选它''与代价。(2) 三法交叉印证 + 数值互验：MH-OR 倒数 $1/0.7527=1.329$ 与 logistic OR 1.323 互验，编码方向对齐正确。(3) 敏感性扎实：三套权重、双查同质、最小格标注、bootstrap 1 万次，全部可复现。(4) 创新落点可被引用、有方法增量：``难度配平比较法''把``公平=同等难度下比''讲成市政能懂的一句话，并进一步把``未达显著''操作化为决策量——功效反算给出达可判定所需样本量（$N\approx3412$）、E-value（$1.566$）量化结论对未观测混杂的脆弱性、TOST 区分``无效应 vs 样本不足''，相对``仅做三法校正''是实打实的方法增量。
...
\item \textbf{样本量与功效}：效应量小（约 4--5 pp / OR$\approx$1.32，Cohen's $h=0.096$），最小格 $n=50$，功效不足——\textbf{功效反算显示达 80\% 功效需总样本约 $N=3412$（现有 700）}，TOST 在 $\pm5$ pp 下亦不能判等效，故属``样本不足''而非``无效应''；``五个角度点估计同号''在效应方向一致而强度微弱时本就常见，不等于显著。
...
\textbf{给市政的决策导向收口}（不确定性下仍可执行的三段式行动规则，仅由结果支持）：

\begin{enumerate}
\item \textbf{立即停用合并按时率做 A/B 考核}——它因案件难度构成不同而系统性误导（6 倍失衡），几乎确定推荐错策略。\textbf{此条结论是显著的、可硬下定论}。
\item \textbf{过渡期默认}：若必须二选一，可暂用``难度配平口径下点估计占优、且对权重稳健（漂移仅 0.7 pp）''的 \textbf{B 作为暂定默认}——但须明示\textbf{暂定、非定论}：B 仅点估计略优约 4--5 pp，三法区间均跨零、未达显著，且方向对未观测混杂不稳健（\textbf{E-value 仅 1.566}）。
\item \textbf{启动可判定的下一步}：对呼叫\textbf{随机化派遣}以根除混杂、支持因果结论；据功效反算，达 80\% 功效检出当前约 4.5 pp 效应\textbf{需总样本约 $N=3412$}（现有 700），即约需扩样本 5 倍或采集一个完整随机化周期。
\end{enumerate}
...
\appendix
\section{附录}
\textbf{复现信息}：代码 \texttt{solve.py}（\texttt{python solve.py}，绝对路径定位数据）；数据 \texttt{data/dispatch.csv}，MD5$=$\texttt{496fd5aafdbbcc3f11edba376d5f2331}（运行时实时校验）；随机种子 \texttt{seed=42}，bootstrap \texttt{N\_BOOT=10000}（\texttt{numpy.random.default\_rng(42)}，分层 4 格各自有放回重采样）。CI 方法：单比例 Wilson；两比例差 Wald；标准化率差 bootstrap 百分位；MH-OR Robins--Breslow--Greenland 方差；logistic Wald CI。弱效应招式箱：功效/样本量反算（\texttt{statsmodels.stats.power}，Cohen's $h$，每组 $N=1706$、合计 3412）；TOST 两单侧检验（边界 $\pm5$/$\pm10$ pp）；E-value（结局常见用 RR$\approx\sqrt{\text{OR}}$，$E=1.566$，区间含零故 E-value(CI)$=1$）。图：\texttt{figures/fig1\_simpson\_reversal.png}、\texttt{fig2\_confounding\_imbalance.png}、\texttt{fig3\_three\_methods\_forest.png}。数字唯一真值源：\texttt{frozen\_numbers.json} / \texttt{results.json}。
