...
**Weaknesses.** The global server adjustment ignores player-level variation. The EWMA α parameter lacks formal optimization. The swing prediction model's binary labeling is threshold-dependent. The permutation test destroys service game structure (though the server-adjusted runs test compensates). Only 31 matches from one tournament are analyzed.
