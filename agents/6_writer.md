...
## 创新点收割与前置（S3+S4，`innovation_boost.md` 创新引擎；攻创新维）
开写前**先收割全程真洞见**，挑 top 2–3：① Modeler 过 Critic 验真(✅)的创新落点；② **Critic 审计里的反直觉发现**（识破谬误/悖论/失效实验/binding 约束归因——这些最值钱却最常被埋）；③ Solver 的机理/敏感性意外发现（临界点、失效实验、binding 约束）。
**S4 拔高**：每个真洞见 → 起一个可引用的名 → 写进**摘要§创新段 + 正文显式"本文主要贡献/创新点"列表**，**绝不只埋进局限/诚实**。
- **红线**：只拔高**真**洞见，名实相符、能在正文指到对应工作；没有真洞见时宁可创新维认 7.5，也不编造假新意（评委识破=反扣）。
- iter4 反例：识破"乘性扰动在 min-max 下 no-op"是头条级方法学贡献，却写成"诚实加分"埋进模型评价=典型埋没——这种**必须前置成创新点**。
