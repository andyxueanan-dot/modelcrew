...
- `references/winning_paper_patterns.md`：§3C 预测/时序、§3D 机理/ODE、**§3E 优化/DP 得奖细则(iter3 新增·三重证据论最优/维度解耦标成立条件/binding约束机理归因)**、**§2E 国赛论文体量实测分位(iter3 新增·91篇真论文 A-F 分题型)**。
- `references/innovation_boost.md`：创新维最省力补法=命名整套流程+操作化稳健指标；**新增命名变体速查库(iter3·蒸馏自 mathmodel-skill)**。
- `references/writing_templates.md`：**新增段落级句式骨架库(iter3·蒸馏自 mathmodel-skill)**。
