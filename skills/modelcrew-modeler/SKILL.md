...
- `references/innovation_boost.md` — **after picking the baseline, mandatorily pass through this**: per-type "one tier up" upgrades (variant / combination / cross-disciplinary) plus citable naming patterns, to lift the innovation dimension without sacrificing interpretability.
  - **Innovation Engine S1 (seed)**: you must give **≥1 real innovation hook** — pick from {cross-disciplinary non-obvious combo / problem-specific new mechanism / self-made interpretable index / substantive improvement to a classic method} (NOT bare naming), state *why it is non-obvious and what it adds over the bare method-box*, tag it `[待验真]` for the Critic's anti-gimmick check, and consult the per-type "innovation hook menu". **Red line**: rather than upgrade, never fabricate fake novelty.
